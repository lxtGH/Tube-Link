...
class Mask2FormerVideoHead(MaskFormerVideoHead):
    ...
    def forward(self, feats, img_metas, return_query=False):
        ...
        # import numpy as np
        # mask_pred[0][0][(-cls_pred[0].softmax(1)[:, 0]).sort()[1]].cpu().sigmoid().numpy() > 0.5
        # for idx, item in enumerate(mask_pred[0][0][(-cls_pred[0].softmax(1)[:, 0]).sort()[1]]):
        #     a = item.cpu().sigmoid().numpy() > 0.5
        #     a = a.astype(np.uint8) * 255
        #     mmcv.imwrite(a, "./tmp/{:06d}_{:.3f}.png".format(idx, (-(-cls_pred[0].softmax(1)[:, 0]).sort()[0])[idx].item()))
        if return_query:
