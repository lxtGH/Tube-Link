...
class TubeLinkVPS(SingleStageDetector):
    ...
    def forward_train(self,
        ...
        key_gt_labels = [ref_gt_labels[0][key_index]]
        key_gt_bboxes = [ref_gt_bboxes[0][key_index]]
        key_gt_masks = [ref_gt_masks[0][:self.split_index]]
        key_gt_semantic = ref_gt_semantic_seg[:, :self.split_index, ] if ref_gt_semantic_seg is not None else None
        key_img_meta = [ref_img_metas[0][:self.split_index]]
        key_instance_ids = [ref_gt_instance_ids[0][key_index]]
        key_instance_ids_match = [ref_gt_instance_ids[0][key_th_index]]

        ref_gt_labels_new = [ref_gt_labels[0][ref_index]]
        ref_gt_bboxes_new = [ref_gt_bboxes[0][ref_index]]
        ...
        ref_instance_ids = [ref_gt_instance_ids[0][ref_index]]
        ref_instance_ids_match = [ref_gt_instance_ids[0][ref_th_index]]
        ...
        key_img_input = ref_img[:, :self.split_index, ]
        ref_img_input = ref_img[:, self.split_index:, ]

        bs, num_frame, three, h, w = key_img_input.size()  # (b,T,3,h,w)
        key_img_input = key_img_input.reshape((bs * num_frame, three, h, w))
        ...
        _ref_gt_bboxes_new[0][:, 0] = _ref_gt_bboxes_new[0][:, 0] - self.split_index
        _ref_gt_labels_new = copy.deepcopy(ref_gt_labels_new)
        _ref_gt_labels_new[0][:, 0] = _ref_gt_labels_new[0][:, 0] - self.split_index
        _ref_instance_ids = copy.deepcopy(ref_instance_ids)
        _ref_instance_ids[0][:, 0] = _ref_instance_ids[0][:, 0] - self.split_index
        ...
        for frame_id in range(num_frame):
            ...
            for i in range(len(result)):
                ...
                if 'ins_results' in result[i]:
                    labels_per_image, bboxes, mask_pred_binary, query_indices = result[i]['ins_results']
                    # add the id in the box field.
                    bboxes = torch.cat([query_indices.to(dtype=bboxes.dtype)[:, None] + 1, bboxes], dim=1)
                    # sort by the score
                    ...
                    result[i]['for_tracker'] = (bboxes, labels_per_image, mask_pred_binary)

                results[i].append(result[i])

        if self.num_stuff_classes == 0:
            results = self.match_instance(results, query_feats, mask_cls_results.cpu(), mask_pred_results.cpu())
        else:
    ...
    def match_panoptic(self, results, query_feats, mask_cls_results, mask_pred_results):
        ...
        # print(tracking_masks.shape)
        # tracking_masks_array = tracking_masks.cpu().data.numpy()
        ...
        # print("generated matched ids:", new_ids)
    ...
    def match_instance(self, results, query_feats, mask_cls_results, mask_pred_results):
        assert len(results) == 1
        assert self.frame_id != -1, "Not initialized"
        result = results[0]
        query_inds = []
        for _frame in result:
            frame_bboxes, lframe_abels_per_image, frame_mask_pred_binary = _frame['for_tracker']
            query_inds.append(frame_bboxes[:, 0].to(dtype=torch.long) - 1)
        query_inds = torch.unique(torch.cat(query_inds))
        query_feats = query_feats[query_inds, 0]

        # get tracking embedding
        track_emb = query_feats.unsqueeze(0)
        for emb_layer in self.embed_fcs:
            track_emb = emb_layer(track_emb)
        track_emb = self.fc_embed(track_emb)
        track_emb = track_emb.squeeze(0)
        ...
        new_ids = self.tracker.match(track_feats)

        new_ids = new_ids + 1000 + 1
        # for indexing
        query_inds = query_inds.tolist()

        result_bs_1 = [{} for _ in range(len(result))]
        for frame_id in range(len(result)):
            _bboxes, _labels_per_image, _mask_pred_binary = result[frame_id]['for_tracker']
            for q_i in range(len(_bboxes)):
                if _bboxes[q_i, 0] - 1 in query_inds:
                    _bboxes[q_i, 0] = new_ids[query_inds.index(_bboxes[q_i, 0] - 1)]
                else:
                    raise ValueError
            _bbox_results = bbox2result(_bboxes, _labels_per_image, self.num_things_classes)
            _mask_results = [[] for _ in range(self.num_things_classes)]
            for j, label in enumerate(_labels_per_image):
                mask = _mask_pred_binary[j].detach().cpu().numpy()
                _mask_results[label].append(mask)
            result_bs_1[frame_id]['ins_results'] = _bbox_results, _mask_results
        ins_results = [result_bs_1]
