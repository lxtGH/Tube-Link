...
class Mask2FormerVideoHeadCustomNoStuff(MaskFormerVideoHead):
    ...
    def __init__(self,
        ...
        self.pre_tracking_query = kwargs.get('pre_tracking_query', False)
        self.pre_tracking_query_list = kwargs.get('pre_tracking_query_list',False)
    ...
    def get_targets_dense(self, cls_scores_list, mask_preds_list, gt_labels_list, gt_masks_list, img_metas):
        (labels_list, label_weights_list, mask_targets_list, mask_weights_list, pos_inds_list, neg_inds_list) = \
            multi_apply(self._get_target_single_dense,
    ...
    def _get_target_single_dense(self, cls_score, mask_pred, gt_labels, gt_masks, img_metas):
        ...
        assigned_gt_inds[:] = 0
        assigned_gt_inds[gt_labels - self.num_things_classes] = torch.arange(
            num_gt, dtype=torch.long, device=mask_pred.device) + 1
        assigned_labels[gt_labels - self.num_things_classes] = gt_labels
        assign_result = AssignResult(num_gt, assigned_gt_inds, None, labels=assigned_labels)
    ...
    def loss_dense(self, cls_scores, mask_preds, gt_labels_list, gt_masks_list, img_metas):
        num_imgs = cls_scores.size(0)
        cls_scores_list = [cls_scores[i] for i in range(num_imgs)]
        mask_preds_list = [mask_preds[i] for i in range(num_imgs)]
        (labels_list, label_weights_list, mask_targets_list, mask_weights_list, num_total_pos, num_total_neg) = \
            self.get_targets_dense(cls_scores_list, mask_preds_list, gt_labels_list, gt_masks_list, img_metas)
        ...
        if self.point_loss:
            ...
            mask_point_preds = F.interpolate(mask_preds.unsqueeze(1), size=(h, w), mode="nearest").squeeze(1).flatten(1,
                                                                                                                      2)
    ...
    def forward_head_video(self, decoder_out, mask_feature, attn_mask_target_size):
        ...
        decoder_out = self.transformer_decoder.post_norm(decoder_out)
        decoder_emb = decoder_out.transpose(0, 1)
        cls_pred = self.cls_embed(decoder_emb)
        mask_embed = self.mask_embed(decoder_emb)
    ...
    def forward(self, feats, img_metas, return_query=False):
        ...
        feature_fpn = []
        for i in multi_scale_memorys:
            feature_fpn.append(i)
        feature_fpn.append(mask_features)
        ...
        cls_pred, mask_pred, attn_mask, decoder_out_emb = self.forward_head_video(
        ...
        for i in range(self.num_transformer_decoder_layers):  # default: 9
            ...
            cls_pred_list.append(cls_pred)
            mask_pred_list.append(mask_pred)
            # only consider the final layer query
            if level_idx == 2:
                emb_pred_list.append(decoder_out_emb)

        if self.pre_tracking_query and self.training:
            if self.pre_tracking_query_list:
                # consider all tracking query list
                query_feat = emb_pred_list
            else:
                # only consider the last one
                query_feat = emb_pred_list[-1]

        if return_query:
            return cls_pred_list, mask_pred_list, query_feat, feature_fpn

        if self.training and self.loss_sem_seg is not None:
            return cls_pred_list, mask_pred_list, query_feat, sem_seg_pred, feature_fpn
        elif self.training and self.loss_sem_seg is None:
            return cls_pred_list, mask_pred_list, query_feat, None, feature_fpn
        else:
            return cls_pred_list, mask_pred_list, feature_fpn
    ...
    def forward_train(self,
        ...
        # forward
        all_cls_scores, all_mask_preds, query_preds, sem_seg_pred, feature_fpn = self.forward(feats, img_metas)

        # preprocess ground truth
        gt_labels, gt_masks = self.preprocess_gt(gt_labels, gt_masks, gt_semantic_seg, gt_instance_ids, img_metas)

        # query based loss
        losses, thing_gt_labels, thing_gt_masks = self.loss(all_cls_scores, all_mask_preds, gt_labels, gt_masks, img_metas)
        ...
        if ref_mode:
            return None, all_cls_scores, all_mask_preds, query_preds, thing_gt_labels, thing_gt_masks, feature_fpn
        else:
            return losses, all_cls_scores, all_mask_preds, query_preds, thing_gt_labels, thing_gt_masks, feature_fpn
    ...
    def loss(self, all_cls_scores, all_mask_preds, gt_labels_list,
        ...
        if self.loss_split_th_st:
            ...
            for gt_labels, gt_masks in zip(gt_labels_list, gt_masks_list):
                ...
                if num_stuff == 0: # a bug in vip seg
                    thing_gt_labels.append(gt_labels[:])
                    thing_gt_masks.append(gt_masks[:])
                else:
            ...
            losses_cls_stuff, losses_mask_stuff, losses_dice_stuff = multi_apply(
                self.loss_dense, stuff_scores, stuff_mask_preds,
        ...
        return loss_dict, thing_gt_labels[-1], thing_gt_masks[-1]
    ...
    def simple_test_with_query(self, feats, img_metas, **kwargs):
        ...
        Returns:
            ...
            - mask_pred_results (Tensor): Mask logits, shape \
                (batch_size, num_queries, h, w).
        """
        all_cls_scores, all_mask_preds, query_feat, feat_fpn = self.forward(feats, img_metas, return_query=True)
        ...
        return mask_cls_results, mask_pred_results, query_feat, feat_fpn