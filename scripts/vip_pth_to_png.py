...
if __name__ == '__main__':
    pth_dir = 'work_dir/results/vipseg'
    path_img = 'data/VIPSeg/images'
    files = sorted(list(mmcv.scandir(pth_dir)))
    palette = [c['color'] for c in CLASSES_HB]
    palette.append([0,0,0])

    video_names = mmcv.list_from_file('data/VIPSeg/val.txt')
    ...
    for file in files:
        ...
        video_folder = os.path.join(path_img, video_names[vid_id])
        ...
        mmcv.imwrite(final_color, 'work_dir/results/vis_vipseg/{:06d}/{:06d}.png'.format(vid_id, img_id))
