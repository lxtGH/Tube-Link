...
CLASSES_HB = [*CLASSES_THING, *CLASSES_STUFF]
NO_OBJ_HB = 255
lam = 0.7
is_visual = False
...
def id2id(id_map, id_mapper):
    assert isinstance(id_map, np.ndarray)
    new_map = np.ones_like(id_map) * NO_OBJ_HB
    for itm in np.unique(id_map):
        if itm < len(id_mapper):
            new_map[id_map == itm] = id_mapper[itm]['id']
        else:
            new_map[id_map == itm] = NO_OBJ_HB
    return new_map

if __name__ == '__main__':
    pth_dir = 'work_dir/results/vss_pth/pred'
    path_img = 'data/VSPW/data'
    files = sorted(list(mmcv.scandir(pth_dir)))
    palette = [c['color'] for c in CLASSES_HB]
    palette.append([0,0,0])

    video_names = mmcv.list_from_file('data/VSPW/test.txt')

    for file in files:
        vid_id, to_split = file.split('_')
        img_id, _ = to_split.split('.')
        vid_id, img_id = int(vid_id), int(img_id)
        video_folder = os.path.join(path_img, video_names[vid_id], 'origin')
        rgb_images = sorted(list(mmcv.scandir(video_folder)))

        map = torch.load(os.path.join(pth_dir, file))
        sem_map = map // DIVISOR
        if is_visual:
            color = id2pale(sem_map, palette)

            rgb_img = mmcv.imread(
                os.path.join(video_folder, rgb_images[img_id])
            )

            final_color = (rgb_img * (1 - lam) + color * lam).astype(np.uint8)
            mmcv.imwrite(final_color, 'work_dir/results/tmpdebug/{}/{}'.format(video_names[vid_id], rgb_images[img_id].replace('.jpg', '.png')))
        else:
            sem_map = id2id(sem_map, CLASSES_HB).astype(np.uint8)
            mmcv.imwrite(sem_map, 'work_dir/results/result_submission/{}/{}'.format(video_names[vid_id], rgb_images[img_id].replace('.jpg', '.png')))
