...
CLASSES_THING = [
    ...
    {'id': 60, 'name': 'person', 'isthing': 1, 'color': (220, 20, 60)},
    {'id': 61, 'name': 'cat', 'isthing': 1, 'color': [255, 82, 0]},
    {'id': 62, 'name': 'dog', 'isthing': 1, 'color': [0, 255, 245]},
    {'id': 63, 'name': 'horse', 'isthing': 1, 'color': [0, 61, 255]},
    {'id': 64, 'name': 'cattle', 'isthing': 1, 'color': [0, 255, 112]},
...
def parse_args():
    ...
    parser.add_argument('--input', type=str, default='data/VIPSeg/images/2385_zhpgbTT2w8c')
    ...
    for task in ['img', 'vis']:
        for img_path, mask_path in zip(img_files, mask_files):
            ...
            if task == 'img':
                ...
                ins_id = ps_id % 10000
                max_ins_id = np.unique(ins_id)[-2]
                ins_id = np.where(ins_id == max_ins_id, max_ins_id, 0)
