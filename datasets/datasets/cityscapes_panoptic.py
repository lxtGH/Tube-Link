...
__all__ = ['CityscapesPanopticDataset', 'CityscapesSTEPPanopticDataset']
...
class COCOPanoptic(COCO):
    ...
    # load_ext for compatibility of nv pycocotools
    def createIndex(self, load_ext=False):
        assert not load_ext
        # create index
...
class CityscapesPanopticDataset(CocoDataset):
    ...
    CLASSES = [
        ...
        'bicycle', 'road', 'sidewalk', 'building', 'wall', 'fence', 'poole', 'traffic light', 'traffic sign', 'vegetation',
        'terran', 'sky'
    ]
    THING_CLASSES = [
        'person', 'rider', 'car', 'truck', 'bus', 'train', 'motorcycle',
    ...
    STUFF_CLASSES = [
        'road', 'sidewalk', 'building', 'wall', 'fence', 'poole', 'traffic light', 'traffic sign', 'vegetation',
        'terran', 'sky'
    ]
    ...
    def load_annotations(self, ann_file):
        ...
        # default stuff: 0-N_{stuff} -1, thing: N_{stuff} - N_{thing+stuff} - 1
        # self.cat2label = {7: 0, 8: 1, 11: 2, 12: 3, 13: 4, 17: 5, 19: 6, 20: 7, 21: 8, 22: 9, 23: 10, 24: 11, 25: 12, 26: 13, 27: 14, 28: 15, 31: 16, 32: 17, 33: 18}
        # To match the coco format -> changes to thing before and stuff after
        self.cat2label = {}
        for i, cat_id in enumerate(self.cat_ids):
            # for stuff class:
            self.cat2label[cat_id] = i + len(self.THING_CLASSES)
            if i >= len(self.STUFF_CLASSES):
                self.cat2label[cat_id] = i - len(self.STUFF_CLASSES)

        # change to thing: 0 - N_{thing} -1, stuff: N_{thing} - N_{thing + stuff} - 1
        # self.cat2label = {7: 8, 8: 9, 11: 10, 12: 11, 13: 12, 17: 13, 19: 14, 20: 15, 21: 16, 22: 17, 23: 18, 24: 0, 25: 1, 26: 2, 27: 3, 28: 4, 31: 5, 32: 6, 33: 7}
        ...
        for i in self.img_ids:
            ...
            info['segm_file'] = origin_name.replace("_gtFine", "_gtFine_panoptic").replace('jpg', 'png').replace("_leftImg8bit", "")
            info['filename'] = origin_name.split("_")[0] + "/" + origin_name.replace("_gtFine", "")
...
@DATASETS.register_module()
class CityscapesSTEPPanopticDataset(CocoDataset):
    """Cityscapes STEP dataset for Panoptic segmentation.
    ...
    STUFF_CLASSES = [
        'road', 'sidewalk', 'building', 'wall', 'fence', 'poole', 'traffic light', 'traffic sign', 'vegetation',
        'terran', 'sky', 'rider', 'truck', 'bus', 'train', 'motorcycle',
        'bicycle'
    ]
    def __init__(self,
                 ann_file,
    ...
    def load_annotations(self, ann_file):
        ...
        for i, cat_id in enumerate(self.cat_ids):
            # For thing class:
            if i == 11:  # person
                self.cat2label[cat_id] = 0
            elif i == 13:  # car
                self.cat2label[cat_id] = 1
            elif i > 11:  # other thing class to stuff class
                self.cat2label[cat_id] = i
            else:  # shift stuff class
                self.cat2label[cat_id] = i + len(self.THING_CLASSES)

        # print(self.cat2label)
        # change to thing: 0 - N_{thing} -1, stuff: N_{thing} - N_{thing + stuff} - 1
        # self.cat2label = {7: 2, 8: 3, 11: 4, 12: 5, 13: 6, 17: 7, 19: 8, 20: 9, 21: 10, 22: 11, 23: 12, 24: 0, 25: 12, 26: 1, 27: 14, 28: 15, 31: 16, 32: 17, 33: 18}
    ...
    def evaluate(self,
        ...
        # Compatible with lowercase 'pq'
        metrics = ['PQ' if metric == 'pq' else metric for metric in metrics]
        allowed_metrics = ['PQ', 'bbox', 'segm', 'proposal', 'mIoU']
        for metric in metrics:
            if metric not in allowed_metrics:
                raise KeyError(f'metric {metric} is not supported')
        ...
        eval_results = {}
        if 'mIoU' in metrics:
            iou_res = self.evaluate_miou(results)
            eval_results.update(iou_res)
            metrics.remove('mIoU')
    ...
    def evaluate_miou(self, results):
        pipeline = Compose([
            dict(type='LoadPanopticAnnotations', with_bbox=False, with_label=False, with_mask=False, with_seg=True)
        ])
        segmentations = []
        for idx in range(len(self)):
            info = self.get_ann_info(idx)
            info = {'ann_info': copy.deepcopy(info)}
            self.pre_pipeline(info)
            seg_map = pipeline(info)
            segmentations.append(seg_map['gt_semantic_seg'])

        predictions = []
        for _result in results:
            pan_map = copy.deepcopy(_result['pan_results'])
            pan_map[pan_map == len(self.CLASSES)] = 255
            pan_map = pan_map % INSTANCE_OFFSET
            predictions.append(pan_map)

        from datasets.datasets.core.seg_evaluation import mean_iou
        miou_res = mean_iou(predictions, segmentations, num_classes=len(self.CLASSES), ignore_index=255)
        return {"mIoU" : miou_res}
