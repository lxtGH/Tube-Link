...
from datasets.datasets.core import eval_vis, results2outs
...
class YouTubeVISDataset(CocoVideoDataset):
    ...
    CLASSES_ovis = ('Person', 'Bird', 'Cat', 'Dog', 'Horse', 'Sheep',
                    'Cow', 'Elephant', 'Bear', 'Zebra', 'Giraffe', 'Poultry',
                    'Giant_panda', 'Lizard', 'Parrot', 'Monkey', 'Rabbit', 'Tiger',
                    'Fish', 'Turtle', 'Bicycle', 'Motorcycle',
                    'Airplane', 'Boat', 'Vehical')

    def __init__(self, dataset_version, inf_len=-1, *args, **kwargs):
    ...
    def set_dataset_classes(cls, dataset_version):
        if dataset_version == '2019':
            ...
            cls.CLASSES = cls.CLASSES_ovis
        else:
            raise NotImplementedError('Not supported YouTubeVIS dataset'
                                      f'version: {dataset_version}')

    def mmdet2mmtrack(self, results):
        results_new = dict()
        results_new['track_bboxes'] = []
        results_new['track_masks'] = []
        for _vid in results:
            if isinstance(_vid, str):
                ...
                vid = _vid
            for frame in vid:
                results_new['track_bboxes'].append(frame['ins_results'][0])
                results_new['track_masks'].append(frame['ins_results'][1])
        results = results_new
        return results

    def format_results(self,
                       results,
                       resfile_path="./work_dirs/",
                       metrics=['track_segm'],
                       save_as_json=True):
        ...
        Returns:
            ...
            files.
        """
        # HY : convert from mmdet to mmtracking
        results = self.mmdet2mmtrack(results)
        # HY : Done.
        ...
        if self.load_all_frames:
            inds = self.inds_when_all_frames
            num_vids = len(inds) - 1
        ...
        for i in range(num_vids):
            ...
            for frame_id, (bbox_res, mask_res) in enumerate(
                ...
                # save the temporal masks with the same ids
